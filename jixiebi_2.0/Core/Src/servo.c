...
IK_Result_t result = {
    .j = {0.0f, 90.0f, 90.0f, 90.0f, 90.0f, 160.0f},
    .ready = 0
};
FK_Result_t res[4];
...
//    if(is_negative_x) r_target = sqrtf(x*x + y*y) + bias_base; //±???????????
...
//    result.j[2] =270.0f - j3_math;//180向前
	result.j[2] =j3_math - 90.0f;//0向前
...
//    result.j[1] =180.0f - j2_math;//180度向前
	result.j[1] =j2_math;//0向前

//    result.j[3] =360.0f - (pitch_deg + result.j[1] + result.j[2]);//180向前
	result.j[3] =180.0f - pitch_deg - result.j[1] - result.j[2];//0向前
...
IK_Result_t IK_Get_Target_Angle(float x, float y, float z,  IK_Mode mode) {
    ...
    for (int i = 0; i < 19; i++) {
        ...
        if (Check_Angle_Valid(res,z)) return res;
        res = IK_Solve_Core(x, y, z, target_pitch - (i*5.0f));
        p_log = target_pitch - (i*5.0f);
        if (Check_Angle_Valid(res,z)) return res;
    }
...
bool Check_Angle_Valid(IK_Result_t r,float z) {
    ...
    if (r.j[2] < -45.0f || r.j[2] > 135.0f) return false;
    if (r.j[3] < 0 || r.j[3] > 180) return false;
	
	float j1_rad = r.j[1] * (M_PI / 180.0f);
    float j2_rad = (90.0f + ( r.j[1] - 90.0f +  r.j[2] - 90.0f)) * (M_PI / 180.0f);
    float j3_rad = (90.0f + ( r.j[1] - 90.0f +  r.j[2] - 90.0f +  r.j[3] - 90.0f)) * (M_PI / 180.0f);
    float x1 = bias_base;
    float z1 = L1;
